...
public class ProfileController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String error = validate(fullname, phone, req);
		if (error != null) {
		...
		try {
			...
			User updated = userService.updateProfile(account.getId(), fullname.trim(),
					phone != null ? phone.trim() : null, images);
	...
	private String validate(String fullname, String phone, HttpServletRequest req) {
		...
		try {
			...
			if (part != null && part.getSize() > 0) {
				String errImg = ValidationUtil.validateImageFile(part.getSubmittedFileName(), part.getSize(),
			...
			return "Không đọc được file ảnh tải lên";
		}
		return null;
	}
